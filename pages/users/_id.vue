<template lang="pug">
  main.main
    section.section-user(v-if="user")
      .section-user__row
        ...
        .section-user__column.section-user__column_right
          ...
          .section-user__publications
            ...
            part-back-nav.section-user__nav(:nav="nav" :status="status" @change="changeStatus" mobile mobileBlue)
            .section-user__publications-list.flex(v-if="list.length > 0" :key="key")
      ...
      modal-product(:product="product" @changeCategory="changeCategory(product.id)"  @status="changeProductStatus" @ban="banUser" @close="GET_ACTIVE_PRODUCT_ID_REMOVE")
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      let search = `/admin/users/${this.$route.params.id}?page=1&searchJoin=and&search=status:${this.status}&with=gallery;city;author`;
