    ...
    section.section-user(v-if="user")
      .section-user__row
        ...
        .section-user__column.section-user__column_right
          ...
          div
          part-back-nav.section-user-inner(:nav="nav" :status="status" @change="changeStatus" mobile mobileBlue)
          .section-user__settings(v-if="status === 1")
            .section-user__settings-list
              ...
              .section-user__settings-item
                ...
                user-change-email.section-user__settings-field(:user="user" :edit="changeEmail" @edit="(val)=>changeEmail=val")
                user-change-phone.section-user__settings-field(:user="user" :edit="changePhone" @edit="(val)=>changePhone=val")
              .section-user__settings-item
          ...
          .section-user__statistics(v-if="status === 2")
            .section-user__statistics-list
              .section-user__statistics-item(v-for="(item,index) in getList" :key="index")
                ...
                .section-user__statistics-item__bottom
                  ...
                  .section-user__statistics-item__icon( v-tippy="{ placement : 'bottom-center',animateFill: false, theme : 'statistics' }"   content="<strong class='section-user__statistics-item__icon-info'>Contrary to popular belief, Lorem Ipsum is not simply random text.</strong>")
    ...
    userList() {
      if (this.user) {
        return [
          ...
          {
            label: 'Date register',
            text: this.format(new Date(this.$newDate(this.user.created_at)), 'dd/MM/yyyy')
          },
          {
            label: 'ID',
            text: this.user.id || '-'
  ...
  methods: {
    ...
    fetchData() {
      let search = `/admin/moderators/${this.$route.params.id}`;``
       ...
       this.$axios.$get(`/admin/moderators/${this.$route.params.id}/statistics`).then(({data}) => {
         this.statistic = data
        // this.user = res
        // this.fields[0].value = res.first_name
        // this.fields[1].value = res.last_name
    ...
    delModal() {
      this.$axios.$delete(`/admin/moderators/${this.$route.params.id}`).then(res => {
