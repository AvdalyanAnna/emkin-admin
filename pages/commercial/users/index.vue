    ...
    section-support-chat-container(:switchers="switchers" title="For users")
      ...
      template(#nav)
        ...
        transition(name="page")
          button-primary.section-users__btn-add.mobile-show(border v-if="showAddButton"  icon="plusBig" :to="`/commercial/users/add?group=${filter.id}`") Add a new
      template(#table)
        part-chat-table.support-chat__table.commercial-table(
          ...
          td
            ...
            .chat-table__date
              .chat-table__text {{format(new Date($newDate(row.created_at)), 'dd/MM/yyyy')}}
          td
            .chat-table-btns
              .chat-table-btns__switcher
                button-primary(icon="close" grayWithText @click.native.prevent="delItem(i)") Delete
                button-primary(v-if="status === 1" grayWithText @click.native.prevent="draftItem(i)") Draft
                button-primary(blue :to="`/commercial/users/${row.id}?group=${filter.id}`") Check
              .chat-table__arrow(v-b-modal.modal-panel-users)
                svg-icon(name="arrowRight")
        list-null(v-else)
        part-chat-sidebar.support-chat__sidebar(:list="list")
      template(#footer)
        button-primary.modal-category__question-btn.ml-0.mobile-noShow(v-if="showAddButton" gray icon="plusBig" :to="`/commercial/users/add?group=${filter.id}`") Add a new
...
export default {
  ...
  methods: {
    ...
    getList(id) {
      ...
      let api = `${this.$api.commercialUsers}?searchJoin=and&period=${this.filter.id}&with=avatar&${sortNewest}&search=status:${this.status}`
       api += `;period_of_stay_id:${this.filter.id || id}`
    ...
    draftItem(index) {
      this.$axios.$put(`${this.$api.commercialUsers}/${this.rows[index].id}`).then(res => {
    ...
    delModal() {
      this.$axios.$delete(`${this.$api.commercialUsers}/${this.deleteId}`)
          .then(res => {
