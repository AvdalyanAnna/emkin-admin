<template lang="pug">
  main.main.bussness
    section-support-chat-container(:switchers="switchers" title="For Business")
      template(#filter)
        part-search-filter.support-chat__filter(:disabled="disabled" @search="searchItems")
          form-select-array-big.commercial__input(
            ...
            @change="changeSelect"
            @more="changeClient"
            :items="filters" v-model="filter" :value="filter"
          )

      template(#nav)
        part-back-nav.support-chat__nav(:nav="nav" :status="status" @change="changeNav")
        transition(name="page")
          button-primary.section-users__btn-add.mobile-show(border v-if="showAddButton"  icon="plusBig" :to="`/commercial/business/add?clientId=${clientId}`") Add a new
      template(#table)
        part-chat-table.support-chat__table.commercial-table(v-if="rows.length> 0"
          @rowClick="clickField"
          :rows="rows" withoutStatus v-slot="{ row, i }")
          td
          ...
          td
            ...
            .chat-table__date
              .chat-table__text {{format(new Date(row.created_at), 'dd/MM/yyyy')}}
          td
            .chat-table-btns
              .chat-table-btns__switcher
                button-primary(icon="close" grayWithText @click.native.prevent="delItem(i)") Delete
                button-primary(v-if="status !== 3" grayWithText @click.native.prevent="draftItem(i)") Draft
                button-primary(blue :to="`/commercial/business/${row.id}?clientId=${clientId}`") Check
              .chat-table__arrow()
                svg-icon(name="arrowRight")
        list-null(v-else)
        part-chat-sidebar.support-chat__sidebar(:list="list")
      template(#footer)
        button-primary.modal-category__question-btn.ml-0.mobile-noShow(v-if="showAddButton" gray icon="plusBig" :to="`/commercial/business/add?clientId=${clientId}`") Add a new
    modal-delete(@cancel="deleteId = null" @deleteItem="delModal")
    modal-add-client(@close="getClients()")
    modal-edit-client(:key="clientKey" @changeClient="changeFilter" :clientId="editClient ? editClient.id : 0" @close="editClient = null")
</template>
...
export default {
  ...
  methods: {
    ...
    changeClient(id) {
      ...
      this.$axios.$get(`/admin/commercial/clients/${id}?with=avatar`).then(({data}) => {
    ...
    clickField(row) {
      if (!this.$screen.lt) {
        this.$router.push(`/commercial/business/${row.id}?clientId=${this.clientId}`)
      }
    },
    getClients() {
    ...
    draftItem(index) {
      this.$axios.$put(`${this.$api.businesses}/${this.rows[index].id}`)
          .then(() => {
    ...
    delModal() {
      this.$axios.$delete(`${this.$api.businesses}/${this.deleteId}`)
          .then(() => {
            const index = this.rows.findIndex(item => item.id === this.deleteId)
            this.rows.splice(index, 1)
            if (this.status === 0) this.counts.vertical_count--
            if (this.status === 1) this.counts.horizontal_count--
