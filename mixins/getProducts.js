...
export default {
  ...
  methods: {
    ...
    getFilters() {
      ...
      if (this.filter1[0].date[0] !== null && this.filter1[0].date[1] !== null) {
        this.filter1[0].value = `${this.format(this.filter1[0].date[0], 'yyyy-MM-dd')},${this.format(this.filter1[0].date[1], 'yyyy-MM-dd')}`
      }
      let price = [{name: 'price', value: ''}]
      if (this.cost.min !== '' && this.cost.max !== '') {
        price[0].value = this.cost.max.split(' ').join('').split(',').join('.') * 1

      }
      return [...this.filter, ...this.filter1, ...price, ...category]
    },
    handleLoadMore($state) {
      this.SET_ADVERTISES(this.getFilters())
          .then(loadState => {
    ...
    banPerson(flag = false) {
      if (!flag) this.BAN_USER()
          .then(() => this.$root.$emit('bv::hide::modal', 'modal-product'))
    ...
    changeCategoryModal(category) {
      this.SET_ACTIVE_PRODUCT(category)
          .then(() => {
            this.SET_CHECKED_ALL(false)
    ...
    showProduct(item) {
      this.$axios.$get(`/admin/products/${item.slug}/details?with=city;author.avatar;category;gallery;answers.filter&`).then(({data}) => {
        this.product = data
