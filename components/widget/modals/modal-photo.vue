  ...
  b-modal(
    ...
    form.modal-photo__row(ref="form")
      .modal-photo__column.modal-photo__column_left
        button-photo.modal-photo__img.modal-photo__img-1(:class="{error:error}" :img="form.file" name="file" big @change="changePhoto")
        ...
        .modal-photo__list
          ...
          .modal-photo__item.h6
            ...
            .modal-photo__item-text {{info.date ? format(new Date($newDate(info.date)),'dd.MM.yyyy') : format(new Date(),'dd.MM.yyyy')}}
          .modal-photo__item.h6
        ...
        .modal-photo__form
          ...
          .modal-photo__buttons
            .modal-photo__buttons-wrap.flex.flex_right
              button-primary.modal-photo__delete(gray v-if="id !== 0" @click.prevent.native="deletePhoto" icon="close" to="#") Delete
...
export default {
  ...
  methods: {
    ...
    deletePhoto() {
      if (this.id) {
        this.$axios.$delete(`${this.$api.gallery}/${this.id}`)
