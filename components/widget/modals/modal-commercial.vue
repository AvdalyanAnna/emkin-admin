  ...
  b-modal(
    ...
    template(#modal-header='{ close }')
      ...
      .modal-commercial__header-row
        ...
        button-switcher.modal-commercial__switcher(:items="switchers" @change="(item) => status = item.status" blue name="commercial")
            ...
            td
              .modal-commercial__date.flex.flex_vertical.flex_justify
                .modal-commercial__date-wrap
                  ...
                  .modal-commercial__date-text {{format(new Date($newDate(row.created_at)), 'dd/MM/yyyy')}}
                .modal-commercial__date-buttons.flex
                  button-medium.modal-commercial__open(blue :to="`${open}/${row.id}`" large) Open
                  button-medium.modal-commercial__delete(gray square @click.native.prevent="delItem(i)" icon="delete" large)
                .modal-commercial__arrow.mobile-visible
                  svg-icon(name="arrowRight")
    template(#modal-footer='{ close }')
      button-primary.modal-commercial__add(light :to="`${open}/add`"  icon="plusBig") Add new
</template>
    ...
    getList() {
      ...
      this.$axios.$get(api).then(res => {
        ...
        res.data.forEach(item => {
          const description = this.users ? JSON.parse(item.description)[0].text : item.description
          data.push({
            id: item.id,
