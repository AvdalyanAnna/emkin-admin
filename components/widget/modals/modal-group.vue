        ...
        .modal-category__row.flex.flex_vertical
          ...
          .modal-category__column.modal-category__column_right
            form-input.modal-login__input(
              placeholder="Enter title" name="first_name"
  ...
  methods: {
    ...
    getNewGroup(flag) {
      if (this.active || this.hasActive) {
        ...
        this.$axios.$get(`/admin/commercial/period-of-stay/${this.active ? this.active.id : this.hasActive }`)
    ...
    save() {
      ...
      if (!this.$v.form.$error && !this.loading) {
        this.loading = true
        this.$axios.setHeader('Accept-Language', this.LANG || 'en')
        this.$axios.$put(this.$api.clientsDays, this.form)
            .then(({data}) => {
              console.log(data)
              this.form = data
    ...
    deleteGroup() {
      this.$axios.$delete(`${this.$api.clientsDays}/${this.form.id}`).then(() => {
        this.$refs['modal-group'].hide();
