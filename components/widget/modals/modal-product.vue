    ...
    .modal-product__row.css-scrollbar(@scroll="onScroll")
      .modal-product__column.modal-product__column_left
        .modal-product__container
          ...
          part-single-gallery.modal-card__gallery(v-if="product" :TestImages="product.gallery")
          part-single-box.modal-product__box(
            ...
            :about="`Registered ${format(new Date($newDate(product.author.created_at)), 'dd.MM.yyyy')}`"
          )

          part-single-content.modal-product__content(v-if="product" :answers="product.answers" :description="product.description || '-'")

          part-single-map.modal-product__map(v-if="product" :coordinates="{longitude:product.longitude,latitude:product.latitude}" :addr="product.address")

      .modal-product__column.modal-product__column_right
        .modal-product__sidebar
          .modal-product__action
            button-primary.modal-product__btn-action(icon="check" :disabled="disabledBtn" blue @click.prevent.native="showActive(product.id)" v-b-modal.modal-activated) Active
            button-primary.modal-product__btn-action(icon="close" :disabled="disabledBtn" red  @click.prevent.native="showRefusal(product.id)") Refuse
            //button-primary.modal-product__btn-action.mobile-visible.svg-m-0(icon="danger" gray  @click.prevent.native="$emit('ban')")
            button-primary.modal-product__btn-action.mobile-visible.svg-m-0(icon="dots" gray v-b-modal.modal-product-action)
          ...
          .modal-product__buttons
              button-primary.modal-product__btn(icon="grid2" gray @click.prevent.native="$emit('changeCategory')") Change categories
          .modal-product__sidebar-bottom
            button-primary.modal-product__btn(icon="danger" border @click.prevent.native="$emit('ban')") Ban this person
...
export default {
  ...
  methods: {
    ...
    onScroll ({ target: { scrollTop, clientHeight, scrollHeight }}) {
      if (scrollTop + clientHeight + 100 >= scrollHeight )  this.disabledBtn = false
    },
    openModal(){
