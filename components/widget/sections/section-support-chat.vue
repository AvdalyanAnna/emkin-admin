  ...
  section.support-chat
    .support-chat__top
      .support-chat__head.flex.flex_vertical.flex_justify
        ...
        button-switcher.support-chat__head-switcher(:items="switchers" @change="changeStatus" blue)
      part-search-filter.support-chat__filter(:disabled="disabled" @search="search" :key="filterKey")
        ...
        @input="disabled = false"
          ...
          transition(name="page" v-if="searchQuery.name.length > 0")
            button-close.modal-search__clear(@click.native.prevent="clearSearch('name')" type="button")
        form-input.search-filter__field(
        name="email"
        label="Email"
        autocomplete="off"
        ...
        @input="disabled = false"
          ...
          transition(name="page" v-if="searchQuery.email.length > 0")
            button-close.modal-search__clear(@click.native.prevent="clearSearch('email')" type="button")
        form-input.search-filter__field(
        name="Description"
        label="Description"
        ...
        @input="disabled = false"
          ...
          transition(name="page" v-if="searchQuery.description.length > 0")
            button-close.modal-search__clear(@click.native.prevent="clearSearch('description')" type="button")
        form-input.search-filter__field(
        ...
        :label="searchQuery.filterDays.label"
          ...
          template(#input)
            ...
            format="DD.MM.YYYY"
            v-model="searchQuery.filterDays.date" range)
          transition(name="page" v-if="searchQuery.filterDays.date[0] !== null")
            button-close.modal-search__clear( @click.native.prevent="clearSearch('filterDays')"  type="button")
      button-action.section-users__btn-filter(circle icon="sliders" v-b-modal.modal-filter v-if="!$screen.st")
      modal-filter(@search="search" @clear="clearSearchMobile()" v-if="!$screen.st")
        .modal-login__field
    ...
    .support-chat__body
      part-chat-table.support-chat__table.commercial-table(:rows="supportThemesAnswers" isChat :activeThemeID="activeThemeID" :activeStatus="activeStatus" @fetchMoreData="handleLoadMoreAnswers" hasName)
      <!--part-chat-sidebar.support-chat__sidebar(:list="list")-->
    .support-chat__bottom
      button-switcher.support-chat__switcher(:items="switchers" @change="changeStatus" name="switchers2"  blue)
</template>
  ...
  export default {
      ...
      searchQuery: {
        handler(newVal,oldVal) {
          this.newSearch = Object.entries(newVal)
              .filter(([key, value]) => typeof newVal[key] !== 'object' && value !== "")
              .map(([key, value]) => `${key}:${value}`)
              .join(";");

          if (this.searchQuery.filterDays.date[0] !== null){
            let startDate = this.format(new Date(this.searchQuery.filterDays.date[0]),'yyyy-MM-dd');
            let endDate = this.format(new Date(this.searchQuery.filterDays.date[1]),'yyyy-MM-dd');
    ...
    computed: {
      ...mapGetters({
        ...
        GET_SUPPORT_THEMES_ANSWER_PAGE: 'support_chat/GET_SUPPORT_THEMES_ANSWER_PAGE',
        GET_SUPPORT_THEMES_ANSWER_LAST_PAGE: 'support_chat/GET_SUPPORT_THEMES_ANSWER_LAST_PAGE',
        GET_SUPPORT_THEMES_ANSWER_LIST: 'support_chat/GET_SUPPORT_THEMES_ANSWER_LIST',
    ...
    methods: {
      ...mapActions({
        ...
        FETCH_SUPPORT_THEMES_ANSWER_LIST: 'support_chat/FETCH_SUPPORT_THEMES_ANSWER_LIST'
      }),
      ...mapMutations({
        SET_ACTIVE_STATUS: 'support_chat/SET_ACTIVE_STATUS',
        SET_ACTIVE_THEM_ID: 'support_chat/SET_ACTIVE_THEM_ID',
        SET_SUPPORT_THEMES_ANSWER_LIST: 'support_chat/SET_SUPPORT_THEMES_ANSWER_LIST',
        SET_SUPPORT_THEMES_ANSWER_PAGE: 'support_chat/SET_SUPPORT_THEMES_ANSWER_PAGE',
        SET_SUPPORT_THEMES_ANSWER_IDENTIFIER: 'support_chat/SET_SUPPORT_THEMES_ANSWER_IDENTIFIER',
        SET_SEARCH_QUERY: 'support_chat/SET_SEARCH_QUERY',
        SET_APPLY_FLAG: 'support_chat/SET_APPLY_FLAG',
      }),
      handleLoadMoreAnswers($state) {
        if (this.supportThemesAnswerPage > this.supportThemesAnswerLastPage) {return}
        ...
        this.FETCH_SUPPORT_THEMES_ANSWER_LIST(params)
            .then(loadState => {
