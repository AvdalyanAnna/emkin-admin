  ...
  section.section-users.checkboxs__list
    .section-users__head.flex.flex_vertical.flex_justify
      .section-users__head-column.section-users__head-column_left
        ...
        button-docs.mobile-visible(v-if="showExel >= 0" to="#" @click.prevent.native="downloadExcel") Download EXCL
      .section-users__head-column.section-users__head-column_right.flex.flex_vertical
        .section-users__sort
          form-checkbox.section-users__sort-checkbox(v-if="!$screen.st" :checked="checkedAll" @change.native="panelSelectedAll")
            p.color-gray Select all
          form-select-array.favorites-ad__select(label="Sort by" :items="sorts" v-model="sort" :value="sort" noBorder)
        button-switcher.section-users__switcher(:items="switchers" @change="changeSwitcher" blue)
    button-primary.section-users__btn-add.mobile-visible(border icon="plusBig" v-if="moderator" v-b-modal.modal-add-moderator) Add moderator
    part-search-filter.annoucement-list__filter(:disabled="search" @search="searchUsers" :key="filterKey")
      form-input.search-filter__field(
        ...
        template(#input)
          transition(name="page" v-if="item.key==='phone'" )
            input.input__field( :id="item.key" type="tel" name="item.key" v-model="item.value" )
    ...
    modal-filter(@search="searchUsers" @clear="clearUsers" v-if="!$screen.st")
      .modal-login__field(v-for="(item,i) in filter " :key="i")
        ...
        form-input(
          ...
          template(#input)
            transition(name="page" v-if="item.key==='phone'" )
              input.input__field(placeholder="Phone" :id="item.key" type="tel" name="item.key" v-model="item.value" )
    ...
    .section-users__bottom.flex.flex_justify
      .section-users__bottom-column
        button-primary.section-users__btn(blue v-if="lastPage !== 1 && page < lastPage" @click.prevent.native="handleLoad") Load more
      .section-users__bottom-column.flex
        button-primary.section-users__btn.section-users__btn-exel(gray icon="plusBig" v-if="moderator" v-b-modal.modal-add-moderator) Add moderator
        button-primary.section-users__btn.section-users__btn-exel(v-if="showExel >= 0" gray icon="download" to="#" @click.prevent.native="downloadExcel") Download base EXCL
    .section-users__panel-wrap(v-if="!$screen.st")
      button-action.section-users__btn-filter(circle icon="sliders" v-b-modal.modal-filter)
    transition(name="fade")
      part-annoucement-panel.section-users__panel(v-if="checkedList.length > 0" @closePanel="checkedAllItems()" :selected="checkedList.length" modalName="modal-panel-users")
        button-medium.annoucement-panel__btn(icon="noneUser" @click.native.prevent="banUserList" blue small) {{status === 1 ?'Un Ban': 'Ban'}}
    modal-panel(id="modal-panel-users")
      .modal-panel__item
        button-border.modal-panel__btn(:icon="status === 1?'user':'noneUser'" @click.native.prevent="banUserList" :blue="status === 1" :gray="status !== 1" BigIcon) {{status === 1 ?'Un Ban': 'Ban'}}
    modal-add-moderator(@close="getModerators")
    modal-ban-person(:title="status === 1 ?'Un Ban': 'Ban'" @ban="banUserModal" @cancel="ban = null" :status="status")
</template>
...
export default {
  ...
  computed: {
    showExel() {
      const permission = 5
      return this.$auth.user.permissions_ids.findIndex(item => item === permission)
    }
  },
  watch: {
    sort() {
  ...
  methods: {
    ...
    fetchData(flag) {
      ...
      let type = 'users'
      if (this.filterDays.date[0] !== null && this.filterDays.date[1] !== null) {
        this.filterDays.value = `${this.format(this.filterDays.date[0], 'yyyy-MM-dd')},${this.format(this.filterDays.date[1], 'yyyy-MM-dd')}`
      }
      if (!this.search && this.$getQueryFilter([...this.filter, this.filterDays]) !== '') searchStr += ';' + this.$getQueryFilter([...this.filter, this.filterDays])
      if (this.moderator) type = 'moderators';
      let search = `admin/${type}?searchFields=phone:like;email:like&searchJoin=and&${this.getSort()}&${searchStr}&page=${this.page + 1}&per_page=20`;
      if (flag) search += '&export=true'

      return this.$axios.$get(search)
    },
    async handleLoad() {
      await this.fetchData().then(({data, meta: {current_page, last_page, total}}) => {
        if (this.page === 0) {
          this.list = []
        }
        data.forEach(item => {
    ...
    downloadExcel() {
      ...
      if (this.moderator) type = 'moderators';
      let search = `admin/${type}?searchJoin=and&${this.getSort()}&${searchStr}&per_page=${this.page * 15}&export=true`;
