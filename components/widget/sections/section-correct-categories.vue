<template lang="pug">
  section.correct-categories.checkboxs__list
    .correct-categories__box
      ...
      draggable.correct-categories__list.flex(v-model="listLevel1"  v-bind="dragOptions" group="people" handle=".handle" @start="drag=true" @end="drag=false")
    ...
    .correct-categories__box.level(v-if="level1")
      ...
      draggable.correct-categories__list.flex(v-model="listLevel2"  v-bind="dragOptions" group="people" handle=".handle" @start="drag=true" @end="drag=false")
    ...
    .correct-categories__box.level(v-if="level2")
      ...
      draggable.correct-categories__list.flex(v-model="listLevel3"  v-bind="dragOptions" group="people" handle=".handle" @start="drag=true" @end="drag=false")
    ...
    transition(name="fade")
      part-annoucement-panel.correct-categories__panel(v-if="checkedList.length > 0" :selected="checkedList.length" @closePanel="closePanel" modalName="modal-correct-categories")
        template(v-if="checkedList.length === 1 ")
          button-medium.annoucement-panel__btn(icon="duplicate" @click.native.prevent="addDuplicatione" blue small) Duplicate
          button-medium.annoucement-panel__btn(icon="plusBig" @click.native.prevent="addItem" blue small) Add category
          button-medium.annoucement-panel__btn(icon="penEdit" @click.native.prevent="editItem" blue small) Edit
        button-medium.annoucement-panel__btn(icon="delete"  v-b-modal:modal-delete blue small) Delete
    modal-panel(id="modal-correct-categories")
      template(v-if="checkedList.length === 1 ")
        .modal-panel__item
          button-border.modal-panel__btn(icon="duplicate" @click.native.prevent="addDuplicatione" BigIcon gray) Duplicate
        .modal-panel__item
          button-border.modal-panel__btn(icon="plusBig" @click.native.prevent="addItem" BigIcon gray) Add category
        .modal-panel__item
          button-border.modal-panel__btn(icon="penEdit" @click.native.prevent="editItem" BigIcon gray) Edit
      .modal-panel__item
        button-border.modal-panel__btn(icon="closeBig" v-b-modal:modal-delete BigIcon gray) Delete
    modal-category(
    ...
    modal-delete(@cancel="$bvModal.hide(`modal-correct-categories`)" @deleteItem="deleteAll")
</template>
...
export default {
  ...
  methods: {
    ...
    changeOrder(array) {
      let orders = []
      array.forEach((item, index) => orders.push({
        id: item.id,
        order: index + 1
      }))
      if (orders.length > 0) {
    ...
    addDuplicatione() {
      const categories = [...this.listLevel1, ...this.listLevel2, ...this.listLevel3, ...this.listLevel4]
