                ...
                slot(:row="row" :i="i")
                  .chat-table-btns
                    .chat-table-btns__switcher
                      button-primary.chat-table__switcher(@click.native.prevent="toChat(row)"   blue  ) Check
                    nuxt-link.chat-table__arrow( :to="`/support-chat/chat/${activeThemeID}/${activeStatus}`")
                      svg-icon(name="arrowRight")
    infinite-loading( :distance="20"  @infinite="infinite" :identifier="GET_SUPPORT_THEMES_ANSWER_IDENTIFIER")
      div(slot="no-results")
      div(slot="no-more")
      div(slot="spinner")
</template>
...
export default {
  ...
  computed: {
    ...mapGetters({
      GET_SUPPORT_THEMES_ANSWER_IDENTIFIER : 'support_chat/GET_SUPPORT_THEMES_ANSWER_IDENTIFIER',
  ...
  methods:{
    ...
    check(row) {
      ...
      if (!this.$screen.lt) {
        this.$router.push(`/support-chat/chat/${this.activeThemeID}/${this.activeStatus}`)
      }
    },
    async toChat(data){
      if (this.activeStatus === this.STATUS_CLOSE || this.activeStatus === this.STATUS_NEW){
