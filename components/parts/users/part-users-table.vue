  ...
  .users-table(:class="Mods")
    table
      thead.color-gray
        tr
          th
            form-checkbox.users-table__checkbox(:checked="checkedAll" @change.native="$emit('allChecked')")
          th
            | User
          th
            | Email
      ...
      tbody
        tr(v-for="(row, i) in rows" :key="i")
          ...
          td
            ...
            .users-table__text.text-wrap {{row.created_at ? format(new Date($newDate(row.created_at)),'dd.MM.yyyy') : '-'}}
          td
            .users-table__id-inner
              ...
              .text-right
                .users-table__switcher
                  button-primary.users-table__switcher(gray @click.native.prevent="$emit('openBan',row)") {{status === 1 ?'Un Ban': 'Ban'}}
                  button-primary.users-table__switcher(blue v-if="moderator && status !== 1" :to="`${moderator ? 'moderators' : 'users'}/${row.id}`") Check
                  button-primary.users-table__switcher(blue v-else-if="moderator===false" :to="`${moderator ? 'moderators' : 'users'}/${row.id}`") Check
