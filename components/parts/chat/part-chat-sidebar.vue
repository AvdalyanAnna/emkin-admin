      ...
      .chat-sidebar__container-wrap
        .chat-sidebar__list
          item-chat.chat-sidebar__item(
            ...
            :time="!isExpectationOrViewed ? format(new Date(item.created_at),'hh mm aaa') : item.last_message.created_at"
          ...
          infinite-loading(:distance="20" :identifier="GET_CHAT_USERS_SIDEBAR_IDENTIFIER" @infinite="infinite")
            div(slot="no-results")
            div(slot="no-more")
            div(slot="spinner")
  ...
  export default {
    ...
    methods: {
      ...
      getUserAvatar(item){
        ...
        return item.participants && item.participants.messageable.avatar ? item.participants.messageable.avatar.original_full_url : '';
      }
    },
    computed: {
      ...mapGetters({
        GET_CHAT_USERS_SIDEBAR_IDENTIFIER: 'support_chat/GET_CHAT_USERS_SIDEBAR_IDENTIFIER',
