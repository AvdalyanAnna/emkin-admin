  ...
  .chat-body
      ...
      .chat-body__row.flex.flex_justify.flex_vertical
        ...
        .chat-body__column.chat-body__column_right.flex(v-if="moderator")
          .chat-body__buttons.flex
            ...
            button-primary.chat-body__btn(v-if="parseInt($route.params.status) === STATUS_NEW && appeal.user" @click.native.prevent="apply" blue) Apply
            button-primary.chat-body__btn(v-if="parseInt($route.params.status) === STATUS_NEW && !appeal.user" @click.native.prevent="closeTicket" blue) Close
            part-another-manager.d-flex.mobile-hidden(v-if="parseInt($route.params.status) === STATUS_EXPECTATION" :is_closed="bodyTopData.status" @closeOrViewed="closeOrViewed")
            ...
            button-medium.chat-body__more(icon="dotsRotate" white square v-b-modal.modal-action v-if="parseInt($route.params.status) === STATUS_EXPECTATION")
    ...
    .chat-body__container
      .chat-body__container-wrap.css-scrollbar(@scroll="onScroll" ref="infoBox")
        infinite-loading( :distance="20"  :direction="'top'" :identifier="GET_CONVERSATION_MESSAGES_IDENTIFIER" @infinite="infinite")
          div(slot="no-results")
          div(slot="no-more")
          div(slot="spinner")
        .chat-body__section(v-for="(message, i) in messages" :key="i" v-if="!isNewOrClose")
          ...
          .chat-body__list
            ...
            item-chat-message.chat-body__item(
              ...
              chat-end
              :time="bodyTopData ? format(new Date(bodyTopData.updated_at),'hh mm aaa') : ''"
        ...
        .chat-body__section(v-if="isNewOrClose")
          .chat-body__list
            ...
            :time="format(new Date(appeal.created_at),'hh mm aaa')"
            :content="[{text:appeal.description}]"
            :gallery="appeal.files"
            )
    .chat-body__bottom
      part-chat-form.chat-body__form(v-if="isExpectationOrViewed && bodyTopData.status" :files="files")

    modal-users-adds(
    v-if="messageable_id"
    :userId="messageable_id"
    )

    modal-action(title="The ad is activated")
      part-another-manager(v-if="parseInt($route.params.status) === STATUS_EXPECTATION" @closeOrViewed="closeOrViewed")
  ...
  export default {
    ...
    computed: {
      ...mapGetters({
        ...
        GET_CONVERSATION_MESSAGES_IDENTIFIER: 'support_chat/GET_CONVERSATION_MESSAGES_IDENTIFIER',
    ...
    methods: {
      ...
      ...mapMutations({
        ...
        SET_USERS_CHAT_SIDEBAR_IDENTIFIER: 'support_chat/SET_USERS_CHAT_SIDEBAR_IDENTIFIER',
        ...
        SET_LAST_MESSAGE_OF_LIST_MAKE_SEEN: 'support_chat/SET_LAST_MESSAGE_OF_LIST_MAKE_SEEN',
        RESET_UNREAD_MESSAGES: 'support_chat/RESET_UNREAD_MESSAGES',
        SET_CONVERSATION_MESSAGES_PAGE: 'support_chat/SET_CONVERSATION_MESSAGES_PAGE',
        SET_CONVERSATION_MESSAGES: 'support_chat/SET_CONVERSATION_MESSAGES',
        SET_CONVERSATION_MESSAGES_IDENTIFIER: 'support_chat/SET_CONVERSATION_MESSAGES_IDENTIFIER',
      ...
      closeOrViewed(value){
        this.CLOSE_OR_VIEWED({active_ticket_id:this.active_ticket_id,value:value}).then(res => {
      ...
      apply() {
        ...
        this.APPLY(this.active_ticket_id)
            .then(({data:{ticket:{conversation:{conversation_id}}}}) => {
              ...
              this.$router.push(`/support-chat/chat/${this.$route.params.theme}/${this.STATUS_EXPECTATION}`);
            });
      ...
      makeSeen() {
        this.$axios.put(this.$supportChat.support.readMessages + `/${this.lastMessageOfList.conversation_id}/readAll`).then(res => {
