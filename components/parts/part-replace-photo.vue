<template lang="pug">
  .replace-photo
    .replace-photo__img(v-if="img || imgDefault")
      img.object-fit(:src="img   ? img : require(`~/assets/img/${imgDefault}`)")
    .replace-photo__buttons.flex
      button-primary.replace-photo__btn(blue icon="penEdit" @click.prevent.native="openGallery")
        //input(type="file" :name="name"  ref="file" @change="getFileObject")
        | Replace Photo
      button-primary.replace-photo__btn(gray square @click.prevent.native="$emit('delete')" icon="close")
</template>
